...
| `Tools__Tools` utility inventory | `Tools` skill + native Hermes tools | Yes — doctrine | `Inference` → configured Hermes provider/tools; memory retrieval → Hindsight/LCM; graph → Hindsight (durable entities, contacts, general/domain knowledge, ordinary factual relationships, and durable learnings) & Cognitive graph (only reviewed values, heuristics, tensions, assumptions, mental models, and projects. Arbitrary Knowledge Archive tags, wikilinks, and related notes do not enter mind.db merely because they are linked or graph-shaped); monitor → process/cron; doctor → live probes. No custom utility clone. |
| `Tools__Containment` | `Containment` skill + constitution §8 + deliberate staging/release review | Adapted | The portable/private boundary is retained. This doctrine/release-review row does not redefine write-time enforcement; `SystemFileGuard` remains separately mapped to Hermes approval/path protection in the existing PreToolGuard and Config rows. |
